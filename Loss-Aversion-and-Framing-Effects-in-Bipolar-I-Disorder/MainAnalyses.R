...
#Age --> quite comparable, 35.91 vs 34.28 in BD vs CTL
aggregate(Bipolar$Age, by=list(group=Bipolar$Group),mean) 
aggregate(Bipolar$Age, by=list(group=Bipolar$Group),sd) 
t.test(Bipolar$Age ~ Bipolar$Group) #p=0.536

# Gender --> Gender proportion equal between groups (1=M, 2=F)
table(Bipolar$Group,Bipolar$Gender)
prop.test(table(Bipolar$Group,Bipolar$Gender),correct=FALSE) #p=0.2

# Ethnicity --> White proportion quite equal between groups
table(Bipolar$Group,Bipolar$Caucasian)
prop.test(table(Bipolar$Group,Bipolar$Caucasian),correct=FALSE) #p=0.9579

#Education --> sig difference: CTL more education years (15.95 vs 17.41)
...
wilcox.test(BipolarEduc$Education ~ BipolarEduc$Group) #p=0.02
...
income=lm(income ~ Group, data=Bipolar) #p=0.186
summary(income)
...
t.test(Bipolar$children ~ Bipolar$Group) #p=0.89
...
t.test(Bipolar$asrm_tot ~ Bipolar$Group) #p=0.18
...
t.test(Bipolar$asrm_tot ~ Bipolar$Gender) #p=0.16
...
t.test(Bipolar$stai_state_brief_tot ~ Bipolar$Group) #p=0.17, not sig different.
...
t.test(Bipolar$stai_state_brief_tot ~ Bipolar$Gender) #p=0.319
...
# quite low loss avesion in overall sample: 2.562 reject choices
summary(Bipolar$LA)
sd(Bipolar$LA)
...
wilcox.test(Bipolar$stai_state_brief_tot ~ Bipolar$LossSeeker) #p=0.337
summary(Bipolar.nls$bdisf_tot)
wilcox.test(Bipolar$bdisf_tot ~ Bipolar$LossSeeker) #p=0.262
